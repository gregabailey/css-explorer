...
.tooltip {
  ...
  `}function p(){const o=document.getElementById("ap-anchor"),i=document.getElementById("ap-playground");if(!o||!i)return;let r=!1,l,s;o.addEventListener("pointerdown",t=>{r=!0;const e=o.getBoundingClientRect();l=t.clientX-e.left,s=t.clientY-e.top,o.setPointerCapture(t.pointerId),o.style.cursor="grabbing"}),o.addEventListener("pointermove",t=>{if(!r)return;const e=i.getBoundingClientRect();let n=t.clientX-e.left-l,a=t.clientY-e.top-s;n=Math.max(0,Math.min(n,e.width-o.offsetWidth)),a=Math.max(0,Math.min(a,e.height-o.offsetHeight)),o.style.left=`${n}px`,o.style.top=`${a}px`,o.style.transform="none"}),o.addEventListener("pointerup",()=>{r=!1,o.style.cursor="grab"})}export{p as init,c as render};
