...
export function init() {
  const sidebarItems = [
    ...
    'Settings', 'Billing', 'Integrations', 'API Keys',
    'Webhooks', 'Logs', 'Alerts', 'Team',
    'Permissions', 'Audit Trail', 'Support', 'Docs',
    'Changelog', 'Status', 'Feedback', 'Account'
  ];

  function buildLayout(container) {
    const items = sidebarItems.map((item, i) =>
      `<div class="fss-sidebar-item">${item}</div>`
    ).join('');
    ...
    container.innerHTML = `
      ...
      <div class="fss-main">
        <div class="fss-sidebar">
          ...
          <div class="fss-sidebar-list">${items}</div>
        </div>
  ...
  const broken = document.getElementById('fss-broken');
  const fixed = document.getElementById('fss-fixed');

  if (broken) buildLayout(broken);
  if (fixed) buildLayout(fixed);
}
