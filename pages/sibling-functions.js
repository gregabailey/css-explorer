...
export function init() {
  const stagger = document.getElementById('si-stagger');
  const rainbow = document.getElementById('si-rainbow');
  const shrink = document.getElementById('si-shrink');

  if (stagger) {
    let html = '';
    for (let i = 1; i <= 6; i++) {
      html += `<div class="si-stagger-item">Item ${i} — delay: ${(i * 0.1).toFixed(1)}s</div>`;
    }
    stagger.innerHTML = html;
  }

  if (rainbow) {
    let html = '';
    for (let i = 1; i <= 12; i++) {
      html += `<div class="si-rainbow-cell">${i}</div>`;
    }
  ...
  if (shrink) {
    let html = '';
    for (let i = 1; i <= 8; i++) {
      html += `<div class="si-shrink-item">${i}</div>`;
    }
    shrink.innerHTML = html;
  }
}
