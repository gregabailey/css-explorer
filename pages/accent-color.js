...
export function init() {
  const picker = document.getElementById('ac-picker');
  const form = document.getElementById('ac-form');
  const swatches = document.getElementById('ac-swatches');
  if (!picker || !form || !swatches) return;

  function setColor(color) {
    form.style.setProperty('--ac-color', color);
    picker.value = color;
    swatches.querySelectorAll('.ac-swatch').forEach(s => {
  ...
  swatches.addEventListener('click', e => {
    const swatch = e.target.closest('.ac-swatch');
