...
export function init() {
  const slider = document.getElementById('il-slider');
  const val = document.getElementById('il-val');
  const text = document.getElementById('il-interactive');
  if (!slider || !val || !text) return;

  slider.addEventListener('input', () => {
    val.textContent = slider.value;
    text.style.setProperty('--il-size', slider.value);
  });
}
