...
export function init() {
  const thumbPicker = document.getElementById('sb-thumb-picker');
  const trackPicker = document.getElementById('sb-track-picker');
  const colorBox = document.getElementById('sb-color-box');
  const gutterToggle = document.getElementById('sb-gutter-toggle');

  if (thumbPicker && trackPicker && colorBox) {
    function updateColors() {
      colorBox.style.scrollbarColor = `${thumbPicker.value} ${trackPicker.value}`;
    }
    thumbPicker.addEventListener('input', updateColors);
    trackPicker.addEventListener('input', updateColors);
  }

  const longText = `Short content. Click "Toggle" to remove extra content.<br><br>Additional content that causes overflow.<br><br>Notice how the content shifts in the left box when the scrollbar appears, but stays stable in the right box.<br><br>More filler...<br><br>Even more...<br><br>End.`;
  const shortText = `Short content. Click "Toggle" to add more.`;
  let expanded = false;

  if (gutterToggle) {
    gutterToggle.addEventListener('click', () => {
      expanded = !expanded;
      const t1 = document.getElementById('sb-no-gutter-text');
      const t2 = document.getElementById('sb-with-gutter-text');
      if (t1) t1.innerHTML = expanded ? longText : shortText;
      if (t2) t2.innerHTML = expanded ? longText : shortText;
    });
  }
}
