...
export function init() {
  ...
  const addBtn = document.getElementById('ss-add-btn');
  const clearBtn = document.getElementById('ss-clear-btn');
  const toastArea = document.getElementById('ss-toast-area');
  const toastBtn = document.getElementById('ss-toast-btn');
  const dialogWrap = document.getElementById('ss-dialog-wrap');
  const dialogBtn = document.getElementById('ss-dialog-btn');

  let itemCount = 0;
  const labels = ['Notification received', 'Task completed', 'File uploaded', 'Settings saved', 'Message sent', 'Update available'];

  if (addBtn && list) {
    addBtn.addEventListener('click', () => {
      const card = document.createElement('div');
      card.className = 'ss-card';
      const label = labels[itemCount % labels.length];
      card.innerHTML = `<h4>${label}</h4><p>Item #${++itemCount} — appeared with @starting-style</p>`;
  ...
  let toastCount = 0;
  if (toastBtn && toastArea) {
    toastBtn.addEventListener('click', () => {
      const toast = document.createElement('div');
      toast.className = 'ss-toast';
      toast.textContent = `Toast #${++toastCount} appeared!`;
      toastArea.appendChild(toast);
      setTimeout(() => toast.remove(), 3000);
    });
  }

  if (dialogBtn && dialogWrap) {
    dialogBtn.addEventListener('click', () => {
      if (dialogWrap.querySelector('.ss-dialog-backdrop')) return;
      const backdrop = document.createElement('div');
      backdrop.className = 'ss-dialog-backdrop';
      backdrop.innerHTML = `
      ...
      `;
      dialogWrap.appendChild(backdrop);

      backdrop.addEventListener('click', e => {
        if (e.target === backdrop || e.target.classList.contains('ss-dialog-close')) {
          backdrop.remove();
        }
      });
    });
